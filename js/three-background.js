// ===========================
// DEEP NEURAL NETWORK BACKGROUND (THREE.JS)
// ===========================
...
export function initThreeJS() {  // ← Fixed: Changed back to initThreeJS()
  ...
  const layers = 8; // Number of layers (depth)
    ...
    for (let a of current) {
      for (let b of next) {
        if (Math.random() < 0.2) {
          ...
          connections.push({ a, b, signal: Math.random(), dir: Math.random() });
        }
      }
    }
  }
  ...
  // --- SIGNAL EFFECTS ---
  const signalColorA = new THREE.Color(0x00ffff);
  const signalColorB = new THREE.Color(0xff66ff);
  ...
  function animate() {
    ...
    // Update connections (simulate signals)
    ...
    // Rotate slowly for depth illusion
